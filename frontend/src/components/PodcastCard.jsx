export default function PodcastCard({ title, author, summary, premium }) {
  return (
    <div className="card-hover-overlay group bg-bg-card rounded-2xl p-[1.2rem] transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer hover:-translate-y-2 hover:bg-[rgba(28,28,36,0.9)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.4),0_0_0_1px_rgba(255,107,53,0.2)]">
      ...
      <div className="relative w-full pt-[100%] bg-gradient-to-br from-[#2a2a35] to-[#3a3a45] rounded-xl mb-4 overflow-hidden">
        {/* Play button */}
        <div className="absolute bottom-2 right-2 w-12 h-12 bg-accent-primary rounded-full flex items-center justify-center opacity-0 translate-y-2.5 transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:translate-y-0 shadow-[0_8px_16px_rgba(255,107,53,0.4)] z-10">
      ...
      <p className="text-[0.85rem] text-text-secondary mb-2 relative z-2">{author}</p>
      <p className="text-[0.8rem] text-text-muted leading-relaxed line-clamp-2 relative z-2 font-serif italic">{summary}</p>
      {premium && (
